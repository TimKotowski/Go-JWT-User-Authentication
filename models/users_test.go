...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Firstname`: `character varying`, `Lastname`: `character varying`, `Email`: `character varying`, `Password`: `character varying`, `CreatedOn`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
