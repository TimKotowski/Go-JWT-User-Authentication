...
func userSignUp(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request ) {
		...
		err := queries.Raw(`INSERT INTO users (firstname, lastname, email, password) VALUES ($1, $2, $3, $4) RETURNING id`, user.Firstname, user.Lastname, user.Email, user.Password).Bind(ctx, db, user)
...
func userLogin(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request ) {
		...
		err := queries.Raw(`SELECT id, firstname, lastname, email, password FROM users WHERE email= $1`, user.Email).Bind(ctx, db, dbUser)
		if err != nil {
